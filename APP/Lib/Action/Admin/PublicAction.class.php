...
class PublicAction extends CommonAction{
    ...
    //原来获取分类列表函数，废弃不用
    public function getShopCategoryList_bak($channel=0){
        $db = D('ShopCategory');
        //将无限级分类中的数据去除来,根据concat(path,'-',id) => 'bpath' 实现排序(很关键)
        if($channel==0){
            $list = $db->field(array('id','name','pid','path',"concat(path,'-',id)"=>'bpath'))->order('bpath')->select();
        }
        else{
            $list = $db->where('channel_id='.$channel)->field(array('id','name','pid','path',"concat(path,'-',id)"=>'bpath'))->order('bpath')->select();
        }
