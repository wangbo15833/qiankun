...
class PublicAction extends CommonAction{
    ...
    public function logout() {
        if(isset($_SESSION[C('USER_AUTH_KEY')])) {
            ...
            unset($_SESSION);
            session_destroy();
            $this->success('登出成功！','/User/Public/login');
        }else {
            $this->error('已经登出！');
        }
    }
    ...
    public function checkLogin(){
        ...
        if(preg_match("/1[3458]{1}\d{9}$/",$_POST['user'])){
    ...
    public function getAreaByPid(){
        $pid=I('param_id');
        $db=M('Area');
        $areas=$db->where('parentid='.$pid)->select();
        $data['areas']=$areas;
        $data['error']=0;
        $this->ajaxReturn($data);
    }
